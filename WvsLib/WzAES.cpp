...
#ifndef _WIN32
unsigned int _rotl(uint32_t value, int shift) { return (value << shift) | (value >> (32 - shift)); }

unsigned int _rotr(uint32_t value, int shift) { return (value >> shift) | (value << (32 - shift)); }
#endif //_WIN32
...
// TODO - test against the attack at http://cr.yp.to/mac/variability1.html and make fixes if
// necessary
...
inline uint8_t GF2_8_mult(uint8_t a, uint8_t b) { // todo - make 4x4 table for nibbles, use lookup
    uint8_t result = 0;
...
uint8_t BitSum(uint8_t byte) { // return the sum of bits mod 2
    byte = (byte >> 4) ^ (byte & 15);
    byte = (byte >> 2) ^ (byte & 3);
...
bool CheckByteSub(bool create) {
    if (CheckInverses(create) == false) return false; // we cannot do this without inverses
    ...
    for (x = 0; x <= 255; x++) {
        ...
        y = BitSum(y & 0xF1) | (BitSum(y & 0xE3) << 1) | (BitSum(y & 0xC7) << 2)
            | (BitSum(y & 0x8F) << 3) | (BitSum(y & 0x1F) << 4) | (BitSum(y & 0x3E) << 5)
            | (BitSum(y & 0x7C) << 6) | (BitSum(y & 0xF8) << 7);
        y = y ^ 0x63;
        if (create == true)
            byte_sub[x] = y;
...
bool CheckInvByteSub(bool create) {
    if (CheckInverses(create) == false) return false; // we cannot do this without inverses
    if (CheckByteSub(create) == false) return false;  // we cannot do this without byte_sub
...
#define compute_one(dest, src2, j, C1, C2, C3, Nb)                                     \
    *(dest + j) = T0[GetByte(src2[j], 0)] ^ T1[GetByte(src2[((j + C1 + Nb) % Nb)], 1)] \
...
#define compute_one_small(dest, src2, j, C1, C2, C3, Nb)                           \
    *(dest + j) = *r_ptr++ ^ T0[GetByte(src2[j], 0)]                               \
...
#define compute_one_inv(dest, src2, j, C1, C2, C3, Nb)                                 \
    *(dest + j) = I0[GetByte(src2[j], 0)] ^ I1[GetByte(src2[((j - C1 + Nb) % Nb)], 1)] \
...
#define compute_one_final(dest, src, j, C1, C2, C3, Nb)                              \
    *dest++ = (T4[GetByte(src[j], 0)]) ^ (T5[GetByte(src[((j + C1 + Nb) % Nb)], 1)]) \
...
#define compute_one_final_inv(dest, src, j, C1, C2, C3, Nb)                          \
    *dest++ = (I4[GetByte(src[j], 0)]) ^ (I5[GetByte(src[((j - C1 + Nb) % Nb)], 1)]) \
...
void AES::Encrypt(const uint8_t * datain, uint8_t * dataout, uint32_t numBlocks, BlockMode mode) {
    ...
    case CBC: {
        uint8_t buffer[64];
        memset(buffer, 0, sizeof(buffer)); // clear out - todo - allow setting the Initialization
                                           // Vector - needed for security
        while (numBlocks) {
            for (uint32_t pos = 0; pos < blocksize; ++pos) buffer[pos] ^= *datain++;
...
void AES::Decrypt(const uint8_t * datain, uint8_t * dataout, uint32_t numBlocks, BlockMode mode) {
    ...
    case CBC: {
        ...
        for (uint32_t pos = 0; pos < blocksize; ++pos) *dataout++ ^= buffer[pos];
        datain += blocksize;
        numBlocks--;
...
void AES::TransformOFB(uint8_t * buffer, uint8_t * iv, int size) {
    ...
    for (uint8_t pos = 0; pos < j; ++pos) *buffer++ ^= temp[pos];
    memcpy(IV, temp, 16);
    numBlocks--;
    while (numBlocks) {
        EncryptBlock(IV, temp);
